        ...
        for task2 in mcs.formatted_tasks:
            for depends_on in task2["dependsOn"]:
                for instance2 in filter(
                    lambda x: x["instance"] != -1,
                    self.get_instances(task2["name"], mcs),
                ):
                    ...
                    for instance1 in task1["value"]:
                        for instance2 in filter(
                            ...
                            prob += (
                                ...
                                - instance1["letEndTime"]
                                - N
                                + N * bool_dep_vars[dep_instances_pair]
                            )
                            prob += (
                                delay_vars[dep_instances_pair]
                                <= instance2["letStartTime"]
    ...
    def get_delay(self, source, dest, N):
        ...
        for link in self.network_delays:
            if link == f"{source['device']}-to-{dest['device']}":
                ...
                comm_delay = (
                    self.network_delays[link]
                    + self.get_device_delay(source["device"])
                    + self.get_device_delay(dest["device"])
                )
                return comm_delay
